...
struct CoordinatorCommenter {
    ...
    func process() -> String {
        ...
        for line in lines {
            ...
            // ── Multiline lazy var continuation ──────────────────────────
            if inLazyComment {
                result.append(indent + "// " + trimmed)
                lazyParenDepth += line.filter { $0 == "(" }.count
                lazyParenDepth -= line.filter { $0 == ")" }.count
                if lazyParenDepth <= 0 { inLazyComment = false }
                continue
            }
            ...
            if !funcBodyStack.isEmpty && braceDepth > funcBodyStack.last! {
                ...
                if willClose && trimmed == "}" {
                    result.append(line)          // closing brace ของ func คงไว้
                } else if !trimmed.isEmpty {
                    result.append(indent + "// " + trimmed)
                } else {
                    result.append(line)          // blank line คงไว้
                }
                braceDepth = newDepth
                continue
            }

            // ── lazy var ที่ไม่ใช้ FactoryAssembler ──────────────────────
            if isLazyVar(trimmed) && !trimmed.contains("FactoryAssembler") {
                result.append(indent + "// " + trimmed)
                let paren = line.filter { $0 == "(" }.count - line.filter { $0 == ")" }.count
                if paren > 0 { inLazyComment = true; lazyParenDepth = paren }
                continue
            }
            ...
            if isFuncDecl(trimmed) && opens > 0 && funcBodyStack.isEmpty {
